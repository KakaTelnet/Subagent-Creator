...
def validate_relative_path(
    ...
    *,
    ...
    if path.is_absolute() or ".." in path.parts:
        ...
        return
    unresolved = plugin_root / path
    resolved_root = plugin_root.resolve()
    resolved = unresolved.resolve()
    try:
        resolved.relative_to(resolved_root)
    except ValueError:
        errors.append(f"`{field}` must resolve inside the plugin root")
        return
    if unresolved.is_symlink():
        errors.append(f"`{field}` must not point through a symbolic link")
...
def validate_plugin(plugin_root: Path) -> list[str]:
    ...
    for field in ("homepage", "repository"):
        validate_https_url(manifest.get(field), field, errors)
    keywords = manifest.get("keywords")
    if keywords is not None:
        if not isinstance(keywords, list) or not all(
            non_empty_string(keyword) and "\n" not in keyword and "\r" not in keyword
            for keyword in keywords
        ):
            errors.append("`keywords` must be an array of non-empty single-line strings")
        elif len(keywords) != len(set(keywords)):
            errors.append("`keywords` entries must be unique")
    ...
    if "skills" in manifest:
        ...
        if skills_value not in {"./skills", "./skills/"}:
            errors.append("`skills` must resolve to the root `./skills/` directory")
        validate_relative_path(
            plugin_root,
            skills_value,
            "skills",
    ...
    component_files = {"apps": "./.app.json", "mcpServers": "./.mcp.json"}
    for field, required_path in component_files.items():
        if field not in manifest:
            continue
        if manifest[field] != required_path:
            errors.append(f"`{field}` must point to the root `{required_path}` file")
        validate_relative_path(
            plugin_root,
            manifest[field],
            field,
